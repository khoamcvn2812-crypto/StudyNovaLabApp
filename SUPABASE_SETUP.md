...
2. In **Authentication → URL Configuration**, set the production site URL to `https://studynovalab.vercel.app` and add the Vercel preview URL as an allowed redirect before testing the preview.
...
3. Keep `https://studynovalab.vercel.app` in the allowed app domains. The application intentionally uses that production URL as its OAuth `redirectTo`.
