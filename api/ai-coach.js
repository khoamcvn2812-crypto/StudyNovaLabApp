...
function send(res, status, code, language = 'vi', details = {}) {
  const messages = {
    ...
    openai_auth_error: ['Dịch vụ AI chưa được cấu hình đúng. Vui lòng báo quản trị viên.', 'The AI service credentials are not configured correctly. Please contact the administrator.'],
  ...
  res.end(JSON.stringify({ error: { code, message: (messages[code] || messages.server_error)[language === 'en' ? 1 : 0], requestId: res.aiRequestId, ...details } }));
}
...
function classifyOpenAIError(error) {
  ...
  if (status === 401 || code.includes('invalid_api_key')) return [502, 'openai_auth_error'];
...
export function createHandler(deps = {}) {
  return async function handler(req, res) {
    res.aiRequestId = String(req.headers['x-vercel-id'] || globalThis.crypto?.randomUUID?.() || `ai-${Date.now().toString(36)}`).slice(0, 128);
    res.setHeader('X-Request-ID', res.aiRequestId);
    Object.entries(NO_STORE).forEach(([key, value]) => res.setHeader(key, value));
    ...
    try {
      ...
      try {
        ...
        return res.end(JSON.stringify({ text, requestId: res.aiRequestId }));
      } catch (error) {
