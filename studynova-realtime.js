...
/* Chinese uses its normalized-table adapter; this legacy record adapter is English-only. */
var STORES={vocabmaster_data_v1:'english_vocabulary',writingvault_data_v1:'english_writing'};
var QUEUE='novalab_realtime_queue_v2',IDS='novalab_realtime_ids_v1',REVS='novalab_realtime_revisions_v1';
...
var PROJECT='mamxrfceltluvlonvsqx',OWNER='studynova:'+PROJECT+':english:local-owner';
...
function accountQueue(uid){return QUEUE+':'+uid}function enqueue(rows){if(!rows.length||!window.SN||!SN.user)return;var uid=SN.user.id,key=accountQueue(uid),q=json(localStorage.getItem(key),[]);if(!Array.isArray(q))q=[];rows.forEach(function(row){row=Object.assign({},row,{target_user_id:uid,subject:'english'});var i=q.findIndex(function(x){return x.entity_type===row.entity_type&&x.entity_id===row.entity_id});if(i<0)q.push(row);else q[i]=row});nativeSet.call(localStorage,key,JSON.stringify(q));scheduleFlush()}
...
function storeFor(type){return type.indexOf('english_vocabulary_')===0?'vocabmaster_data_v1':type.indexOf('english_writing_')===0?'writingvault_data_v1':''}
function arrayName(type){return type.replace(/^(english_vocabulary|english_writing)_/,'').replace(/_([a-z])/g,function(_,c){return c.toUpperCase()})}
...
async function flush(){
  clearTimeout(flushTimer);if(!navigator.onLine||!SN.client||!SN.user)return;
  var uid=SN.user.id,key=accountQueue(uid),deviceId=SN.deviceId;var q=json(localStorage.getItem(key),[]);if(!Array.isArray(q)||!q.length)return;var rows=q.filter(function(x){return x.target_user_id===uid&&x.subject==='english'}).map(function(x){var row=Object.assign({},x,{user_id:uid,source_device_id:deviceId,updated_at:new Date().toISOString()});delete row.target_user_id;delete row.subject;return row});
  ...
  var sent=new Set(rows.map(function(x){return recordKey(x.entity_type,x.entity_id)+':'+x.revision}));var latest=json(localStorage.getItem(key),[]).filter(function(x){return !sent.has(recordKey(x.entity_type,x.entity_id)+':'+x.revision)});nativeSet.call(localStorage,key,JSON.stringify(latest))
}
...
async function start(){
  if(!window.SN||!SN.client||!SN.user||startedFor===SN.user.id)return;var nextUser=SN.user.id;if(startedFor){Object.keys(STORES).forEach(function(k){nativeSet.call(localStorage,'studynova:'+PROJECT+':'+startedFor+':english:'+k,localStorage.getItem(k)||'{}')})}stop();var owner=localStorage.getItem(OWNER);applying=true;try{Object.keys(STORES).forEach(function(k){var scoped=localStorage.getItem('studynova:'+PROJECT+':'+nextUser+':english:'+k);if(scoped!==null)nativeSet.call(localStorage,k,scoped);else if(owner&&owner!==nextUser)nativeSet.call(localStorage,k,'{}')});nativeSet.call(localStorage,OWNER,nextUser)}finally{applying=false}startedFor=nextUser;SN.deviceId=(function(){var id=localStorage.getItem('novalab_device_id_v1');if(!id){id=uuid();nativeSet.call(localStorage,'novalab_device_id_v1',id)}return id})();
...
function persistAccountCache(){if(!startedFor)return;Object.keys(STORES).forEach(function(k){nativeSet.call(localStorage,'studynova:'+PROJECT+':'+startedFor+':english:'+k,localStorage.getItem(k)||'{}')})}function stop(){if(channel&&window.SN&&SN.client)SN.client.removeChannel(channel);channel=null;startedFor=''}
window.addEventListener('online',function(){scheduleFlush(0)});window.addEventListener('pagehide',stop);
window.addEventListener('studynova-auth-change',function(){if(SN.user)start().catch(console.warn);else{persistAccountCache();stop()}});
