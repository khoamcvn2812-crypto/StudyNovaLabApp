...
test('returns successful no-store response without accepting a model from client',async()=>{
  ...
  const res=response(); await createHandler(deps)(request(validBody),res); const body=JSON.parse(res.body); assert.equal(res.statusCode,200); assert.equal(body.text,'Safe answer'); assert.ok(body.requestId); assert.equal(res.headers['Cache-Control'],'no-store, no-cache, must-revalidate'); assert.equal(params.model,'configured-model'); assert.equal(params.store,false);
});
...
test('frontend distinguishes JSON errors, HTML rewrites, and empty bodies',async()=>{
  const js=fs.readFileSync(new URL('../studynova-ai-coach.js',import.meta.url),'utf8');
  const context={window:{addEventListener(){}},document:{getElementById(){return null},querySelectorAll(){return[]},addEventListener(){}},localStorage:{getItem(){return'vi'}},console,Response,TypeError,AbortController,setTimeout,clearTimeout};
  ...
  const parse=context.window.StudyNovaAiDiagnostics.parseResponse;
  await assert.rejects(()=>parse(new Response('<html>fallback</html>',{status:200,headers:{'Content-Type':'text/html'}})),error=>error.code==='invalid_content_type'&&/rewrite/.test(error.message));
  await assert.rejects(()=>parse(new Response('',{status:502,headers:{'Content-Type':'application/json'}})),error=>error.code==='empty_http_response');
  await assert.rejects(()=>parse(new Response(JSON.stringify({error:{code:'quota_exceeded',message:'Hết lượt',requestId:'req-1'}}),{status:429,headers:{'Content-Type':'application/json'}})),error=>error.code==='quota_exceeded'&&error.requestId==='req-1');
});
test('frontend renders model text safely and clears session history on account change',()=>{
  ...
  assert.match(js,/finally\{controller=null;busy\(false\)\}/); assert.match(js,/error instanceof TypeError\?copy\[lang\(\)\]\.network/);
});
