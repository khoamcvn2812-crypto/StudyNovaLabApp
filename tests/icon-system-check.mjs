...
const files=['english.html','studynova_writing_vault.html','studynova-auth.js','studynova-realtime.js','writing-drafts.js','service-worker.js','manifest.webmanifest','assets/icons/studynova-icons.css','assets/icons/studynova-icons.js'];
...
for(const page of ['english.html','studynova_writing_vault.html']){const html=fs.readFileSync(new URL('../'+page,import.meta.url),'utf8');if(!html.includes('<sn-icon name="home"')||!html.includes('studynova-icons.js'))throw new Error(`${page}: static SVG icon integration missing`)}
const worker=fs.readFileSync(new URL('../service-worker.js',import.meta.url),'utf8');if(!worker.includes('studynova-platform-v34')||!worker.includes('studynova-icons.svg'))throw new Error('Icon cache shell/version is stale');
